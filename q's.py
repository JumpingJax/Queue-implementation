...
class BoundedQueue: 
    ...
    def dequeue(self):        
        ...
        if self.is_empty()==False:
            front_item=self.__items.pop(0)
            return front_item
        
        else:
            raise Exception('Queue is empty cannot pop')
...
def main():
    ...
    bq.enqueue('matthew')
    try:
        # add one to make it full then try to add one more
        bq.enqueue('chad')
    except Exception as enqueueError:
        print('Cannot enqueue a full queue')
    print(repr(bq))
    print("There are", bq.size(), "elements in my bounded queue.")
    print('----------------------------------')
    '''
    ...
    # no all we get is a reference to the method capacity we cannot reference it directly as it does not exist in the rest of the function outside of the class 
    print(bq.capacity)
    #print(bq.__capacity)
    
    
    # now we test the circular queue 
    # Test bounded queue creation
    cq=CircularQueue(3)
